...
#define FEBIRD_PTREE_METHOD_IMPL_get(Boxer, Boxed) \
...
//--------------------------------------------------------------------
//FEBIRD_PTREE_METHOD_IMPL_get(obj_bool   ,   bool)
FEBIRD_PTREE_METHOD_IMPL_get(obj_int    ,   signed int)
FEBIRD_PTREE_METHOD_IMPL_get(obj_uint   , unsigned int)
FEBIRD_PTREE_METHOD_IMPL_get(obj_long   ,   signed long)
FEBIRD_PTREE_METHOD_IMPL_get(obj_ulong  , unsigned long)
FEBIRD_PTREE_METHOD_IMPL_get(obj_llong  ,   signed long long)
FEBIRD_PTREE_METHOD_IMPL_get(obj_ullong , unsigned long long)
FEBIRD_PTREE_METHOD_IMPL_get(obj_double , float)
FEBIRD_PTREE_METHOD_IMPL_get(obj_double , double)
FEBIRD_PTREE_METHOD_IMPL_get(obj_ldouble, long double)
...
#define FEBIRD_PTREE_METHOD_IMPL_set(Boxed) \
	void ptree::set(fstring path, Boxed val) { \
		set(path, OnNode_create<Boxed>(path, val)); \
	}
//----------------------------------------------------------

FEBIRD_PTREE_METHOD_IMPL_set(  signed int)
FEBIRD_PTREE_METHOD_IMPL_set(unsigned int)
FEBIRD_PTREE_METHOD_IMPL_set(  signed long)
FEBIRD_PTREE_METHOD_IMPL_set(unsigned long)
FEBIRD_PTREE_METHOD_IMPL_set(  signed long long)
FEBIRD_PTREE_METHOD_IMPL_set(unsigned long long)
FEBIRD_PTREE_METHOD_IMPL_set(     float)
FEBIRD_PTREE_METHOD_IMPL_set(     double)
FEBIRD_PTREE_METHOD_IMPL_set(long double)
...
FEBIRD_BOXING_OBJECT_DERIVE_IMPL(php_array, php_array_base)
...
#define GEN_INT_FUNC(Int) \
	...
	void php_output_json_##Int(const obj* self, std::string* out) {  \
		const obj_##Int* x = static_cast<const obj_##Int*>(self); \
		char buf[32] = {0}; \
		size_t len = int_to_str(buf, x->t); \
		out->append(buf, len); \
	} \
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	void php_output_json_bool(const obj* self, std::string* out) {
    ...
    std::string json_escape_string(const char *str) {
        ...
        *ptr2++=0;
        std::string ret_str(out);
        delete[] out;
        return ret_str;
    }

	void php_output_json_string(const obj* self, std::string* out) {
		const obj_string* x = static_cast<const obj_string*>(self);
		out->append("\"");
        //printf("php_output_json_string11\t%s\n", out->c_str());
        // json需要对一些字符串转义
		out->append(json_escape_string(x->c_str()));
        //printf("php_output_json_string22\t%s\n", out->c_str());
		out->append("\"");
        //printf("php_output_json_string33\t%s\n", out->c_str());
	}

	void php_output_json_double(const obj* self, std::string* out) {
		const obj_double* x = static_cast<const obj_double*>(self);
		char buf[96];
		out->append(buf, sprintf(buf, "%f", x->t));
	}

	void php_output_json_ldouble(const obj* self, std::string* out) {
		const obj_ldouble* x = static_cast<const obj_ldouble*>(self);
		char buf[96];
		out->append(buf, sprintf(buf, "%Lf", x->t));
	}

	void php_output_json_array(const obj* self, std::string* out) {
		const obj_array* x = static_cast<const obj_array*>(self);
		out->append("[");
		for (long i = 0, n = x->size(); i < n; ++i) {
			php_append_json((*x)[i].get(), out);
            if (i != n - 1)
              out->append(",");
		}
		out->append("]");
	...
	void php_output_json_php_object(const obj* self, std::string* out) {
...
// O:strlen(object name):object name:object size:{s:strlen(field name):field name:field definition;(repeated per field)}
		...
		for (long i = 0, n = x->fields.end_i(); i < n; ++i) {
            ...
            out->append(":");
			php_append_json(x->fields.val(i).get(), out);
            if (i != n - 1)
              out->append(",");
		}
		out->append("}");
	...
	struct MetaInitializerClass {
		...
		#define ADD_META(type_sig) \
			do { \
				...
				mi.to_json = php_output_json_##type_sig; \
			...
			g_meta[typeid(php_array ).name()].to_json = php_output_json_php_array;
			g_meta[typeid(php_object).name()].to_json = php_output_json_php_object;
...
#define THROW_INPUT_ERROR2(msg, ptr) \
	do{ char buf[256]; \
		snprintf(buf, sizeof(buf), "%s at pos: %ld", msg, long(ptr-base)); \
...
#define THROW_INPUT_ERROR(msg) THROW_INPUT_ERROR2(msg, beg)
	switch (*beg) {
		...
		case 'b': { // b:val;
			...
			char* pp = NULL;
			long val = strtoll(beg+1, &pp, 10);
			if (';' != *pp)
				THROW_INPUT_ERROR2("php_load: boolean, expect ';'", pp);
			beg = pp + 1;
			return new obj_bool(val);
		}
#if 0
		case 'd': { // d:val;
			if (++beg >= end)
