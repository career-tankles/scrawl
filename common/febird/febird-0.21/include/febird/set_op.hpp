...
public:
	...
	void resize_cache(ptrdiff_t size) {
		if (m_cache)
			delete []m_cache;
...
public:
	template<class Value>
	const Key& get_key(const Value& x) const {
		return (*this)(x);
	}
};
...
protected:
	...
	inline void get_cache_key(ptrdiff_t nth/*, tag_cache_none*/) const
	{
		// do nothing...
	...
	inline void load_cache_item(ptrdiff_t, const InputIter&)
	{
		// do nothing...
	}

	inline void resize_cache(ptrdiff_t)
	{
		// do nothing...
	}

// just used by LoserTree::start_yan_wu
// has no this function, LoserTree::start_yan_wu don't support cache_none
// 	void set_cache_item(ptrdiff_t i, const cache_item_type&)
// 	{
// 		// do nothing...
// 	}
...
protected:
	//! 这个函数必须要带 cache_category 参数
	inline const KeyType& get_cache_key(ptrdiff_t nth, tag_cache_key) const
	{
		return this->m_cache[nth];
	}

	inline void load_cache_item(ptrdiff_t i, InputIter iter)
	{
		this->m_cache[i] = this->get_key(*iter);
	}

	// just used by LoserTree::start_yan_wu
	void set_cache_item(ptrdiff_t i, const KeyType& item)
	{
		this->m_cache[i] = item;
	}
};
...
protected:
	//! 这个函数必须要带 cache_category 参数
	inline const KeyType& get_cache_key(ptrdiff_t nth, tag_cache_value) const
	{
		return this->get_key(this->m_cache[nth]);
	}
	...
	// just used by LoserTree::start_yan_wu
	void set_cache_item(ptrdiff_t i, const value_type& item)
	{
		this->m_cache[i] = item;
	}
};
...
public:
	bool comp_value(const value_type& x, const value_type& y) const
	{
		return static_cast<const MultiWay*>(this)->m_comp(
			   static_cast<const MultiWay*>(this)->get_key(x),
			   static_cast<const MultiWay*>(this)->get_key(y));
	}
	...
	{
		for (; !static_cast<MultiWay*>(this)->empty(); ++*this, ++dest)
		{
			*dest = **this;
		}
		return dest;
	...
	{
		while (!static_cast<MultiWay*>(this)->empty())
		{
			value_type x = **this;
			if (cond(x))
				*dest = x, ++dest;
	...
	{
		assert(!static_cast<MultiWay*>(this)->empty());
		...
		for (;;)
		{
			...
			do {
				++n; ++*this;
				if (febird_unlikely(static_cast<MultiWay*>(this)->empty()))
					return dest;
				value_type y = **this;
				if (cond(n, x, y))
					*dest++ = x;
	...
	_OutIt copy_equal(_OutIt dest, DataExtractor dataEx)
	{
		...
		const key_type k = static_cast<MultiWay*>(this)->get_key(x);
		value_type y = x;
		do {
			*dest = dataEx(y); ++dest;
			++*this;
			if (febird_unlikely(static_cast<MultiWay*>(this)->empty()))
				break;
			y = **this;
		} while (!comp_key(k, static_cast<MultiWay*>(this)->get_key(y)));
	...
	ptrdiff_t skip_equal()
	{
		...
		key_type k = static_cast<MultiWay*>(this)->get_key(**this);
		ptrdiff_t n = 0;
		do {
			++*this; ++n;
		} while (!static_cast<MultiWay*>(this)->empty() &&
				 !comp_key(k, static_cast<MultiWay*>(this)->get_key(**this)));
		return n;
	}

	ptrdiff_t skip_equal_eofm()
	{
		assert(!static_cast<MultiWay*>(this)->empty());
		...
		do {
			++*this; ++n;
		} while (!comp_key(k, static_cast<MultiWay*>(this)->get_key(**this)));
...
private:
	...
	{
		if (static_cast<MultiWay*>(this)->empty())
			return dest;
		ptrdiff_t ways = static_cast<MultiWay*>(this)->total_ways();
		key_type  x = static_cast<MultiWay*>(this)->get_key(**this);
		for (;;)
		{
			key_type y;
			ptrdiff_t i = 0;
			do {
				...
				y = static_cast<MultiWay*>(this)->get_key(**this);
...
public:
	...
	ptrdiff_t intersection_size_eofm(const key_type& eofm)
	{
		if (static_cast<MultiWay*>(this)->empty())
			return 0;
		key_type x = static_cast<MultiWay*>(this)->get_key(**this);
	...
	ptrdiff_t unique_size() { return union_size(); }
	ptrdiff_t unique_size_eofm(const key_type& eofm) { return union_size_eofm(eofm); }
	...
	ptrdiff_t union_size_eofm(const key_type& eofm)
	{
		if (static_cast<MultiWay*>(this)->empty())
			return 0;
		key_type x = static_cast<MultiWay*>(this)->get_key(**this);
	...
	template<class KeyToEqualCountIter>
	KeyToEqualCountIter key_to_equal_count(KeyToEqualCountIter iter, ptrdiff_t minEqualCount)
	{
		if (febird_unlikely(static_cast<MultiWay*>(this)->empty()))
			return iter;
		key_type x = static_cast<MultiWay*>(this)->get_key(**this);
	...
	//! @param minEqualCount only output keys which happen minEqualCount times
	template<class KeyToEqualCountIter>
	KeyToEqualCountIter key_to_equal_count_eofm(KeyToEqualCountIter iter, ptrdiff_t minEqualCount, const key_type& eofm)
	{
		if (febird_unlikely(static_cast<MultiWay*>(this)->empty()))
			return iter;
		key_type x = static_cast<MultiWay*>(this)->get_key(**this);
...
template< class way_iter_t
		...
		>
class LoserTree :
	public CacheStrategy< way_iter_t
						, KeyType
						, KeyExtractor
						, HowCache
						>,
	public MultiWay_SetOP< typename std::iterator_traits<way_iter_t>::value_type
						 , KeyType
						 , LoserTree<way_iter_t, KeyType, StableSort, Compare, KeyExtractor, HowCache>
						 >,
	protected Compare
{
	DECLARE_NONE_COPYABLE_CLASS(LoserTree)

	typedef CacheStrategy<way_iter_t
						, KeyType
						, KeyExtractor
						, HowCache
						>
	...
	friend class MultiWay_SetOP< typename std::iterator_traits<way_iter_t>::value_type
						 , KeyType
						 , LoserTree<way_iter_t, KeyType, StableSort, Compare, KeyExtractor, HowCache>
						 >;
public:
...
public:
	...
	const std::vector<way_iter_t>& ways() const { return m_ways; }
	std::vector<way_iter_t>& ways() { return m_ways; }
	...
	void start()
	{
		ptrdiff_t len = m_ways.size();
		if (febird_unlikely(len <= 0))
			throw std::invalid_argument("LoserTree: way sequence must not be empty");

		{
			ptrdiff_t* t = (ptrdiff_t*)realloc(m_tree, sizeof(ptrdiff_t) * len);
		...
		ptrdiff_t minInnerToEx = len / 2;

		std::vector<ptrdiff_t> winner(len);
					  ...
					  const cache_item_type& max_item)
	{
		...
		ptrdiff_t len = ptrdiff_t(m_ways.size());
	...
	bool empty() const
	{
		...
		return !static_cast<const Compare&>(*this)(cur_key, m_max_key); // cur_key >= max_value
	}
	...
	const ptrdiff_t* get_tree() const { return m_tree; }
	...
	ptrdiff_t skip_equal()	{ return this->skip_equal_eofm(); }

	template<class _OutIt>
	_OutIt intersection(_OutIt dest) { return this->intersection_eofm(dest, m_max_key); }
	ptrdiff_t intersection_size() { return this->intersection_size_eofm(m_max_key); }

	template<class _OutIt>
	_OutIt union_set(_OutIt dest) { return this->union_set_eofm(dest, m_max_key); }
	ptrdiff_t union_size() { return this->union_size_eofm(m_max_key); }

	template<class _OutIt>
	_OutIt unique(_OutIt dest) { return this->union_set_eofm(dest, m_max_key); }
	ptrdiff_t unique_size() { return this->union_size_eofm(m_max_key); }
...
protected:
	void ajust(ptrdiff_t s)
	{
		ptrdiff_t parent = ptrdiff_t(s + m_ways.size()) >> 1;
		ptrdiff_t*ptree =  m_tree;
		while (parent > 0)
		{
			ptrdiff_t& tparent = ptree[parent];
	...
	void exter_loser_winner(ptrdiff_t& loser, ptrdiff_t& winner, ptrdiff_t parent, ptrdiff_t len) const
	{
		ptrdiff_t left  = 2 * parent - len;
		ptrdiff_t right = left + 1;
		get_loser_winner(loser, winner, left, right);
	}
	void inner_loser_winner(ptrdiff_t& loser, ptrdiff_t& winner, ptrdiff_t parent, const std::vector<ptrdiff_t>& winner_vec) const
	{
		ptrdiff_t left  = 2 * parent;
		ptrdiff_t right = 2 * parent + 1;
		left = winner_vec[left];
		right = winner_vec[right];
		get_loser_winner(loser, winner, left, right);
	}
	void get_loser_winner(ptrdiff_t& loser, ptrdiff_t& winner, ptrdiff_t left, ptrdiff_t right) const
	{
	...
	{
		assert(m_head < ptrdiff_t(m_ways.size()));
		return *m_ways[m_head];
	}
	...
	inline const KeyType& get_cache_key(ptrdiff_t nth, tag_cache_none) const
	{
		return this->get_key(*m_ways[nth]);
	}

	template<class CacheCategory>
	inline bool comp_cache_item(ptrdiff_t x, ptrdiff_t y,
	...
	bool comp_key_stable(ptrdiff_t x, ptrdiff_t y, const KeyType& kx, const KeyType& ky,
						 boost::mpl::true_ hasTriCompare) const
	{
		ptrdiff_t ret = static_cast<const Compare&>(*this).compare(kx, ky);
		if (ret < 0)
			return true;
		if (ret > 0)
			return false;
		ret = static_cast<const Compare&>(*this).compare(kx, m_max_key);
		assert(ret <= 0);
		if (0 == ret)
			return false;
		else
	...
	{
		if (static_cast<const Compare&>(*this)(kx, ky))
			return true;
		if (static_cast<const Compare&>(*this)(ky, kx))
			return false;
		if (!static_cast<const Compare&>(*this)(kx, m_max_key)) // kx >= max_key --> kx == max_key
		{ // max_key is the max, so must assert this:
			assert(!static_cast<const Compare&>(*this)(m_max_key, kx));
			return false;
		}
		else return x < y;
	}
...
protected:
	ptrdiff_t*m_tree;
	ptrdiff_t m_head; // access m_tree[0] need an extra memory load
	KeyType   m_max_key;
...
struct Heap_WayIterEx : public std::pair<_InIt, _InIt>
{
	...
	Heap_WayIterEx(ptrdiff_t index, const _InIt& i1, const _InIt& i2)
...
template<class _InIt>
Heap_WayIterEx<_InIt> make_way_iter_ex(ptrdiff_t index, const _InIt& i1, const _InIt& i2)
{
...
private:
	...
	bool comp_key_stable(ptrdiff_t x, ptrdiff_t y, const value_type& kx, const value_type& ky,
						 boost::mpl::true_ hasTriCompare) const
	{
		ptrdiff_t ret = m_comp.compare(kx, ky);
		if (ret < 0)
			return true;
		if (ret > 0)
			return false;
	...
	bool is_any_way_end() const
	{
		return (ptrdiff_t)m_range.size() != m_old_size;
	}

	void assertNotEmpty()
	{
		for (ptrdiff_t i = 0, n = m_range.size(); i < n; ++i)
		{
...
public:
	...
	public:
		...
		//! 反转输入参数，用做堆操作的比较
		inline bool operator()(ptrdiff_t x, ptrdiff_t y) const
		{
...
RandIterOfInput{pair}    first                                  second
	...
	HeapMultiWay(RandIterOfInput first, ptrdiff_t length,
				 const Compare& comp = Compare(),
				 const KeyExtractor& keyExtractor = KeyExtractor())
	{
			  ...
			  const KeyExtractor& keyExtractor = KeyExtractor())
	{
		...
		for (RandIterOfInput i = first; i != last; ++i)
		{
			if (i->first != i->second)
			{
				ptrdiff_t way_idx = i - first;
				m_heap.push_back(way_idx);
				// read first value from every sequence
				this->load_cache_item(way_idx, i->first);
			}
		}
...
protected:
	...
	bool is_any_way_end() const
	{
		return (ptrdiff_t)m_heap.size() != m_old_size;
	}

	ptrdiff_t total_ways() const { return m_old_size; }

protected:
	RandIterOfInput  m_beg;
	std::vector<ptrdiff_t> m_heap;
	ptrdiff_t			 m_old_size;
	Compare		     m_comp;
};
