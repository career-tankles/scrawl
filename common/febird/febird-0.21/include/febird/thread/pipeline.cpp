...
#include <febird/circular_queue.hpp>
#include <febird/num_to_str.hpp>
#include <febird/util/compare.hpp>
...
PipelineStep::ThreadData::ThreadData() : m_run(false) {
	m_thread = NULL;
}
PipelineStep::ThreadData::~ThreadData() {}
...
void PipelineStep::start(int queue_size)
{
	...
	for (size_t threadno = 0; threadno != m_threads.size(); ++threadno)
	{
		...
		//
		m_threads[threadno].m_thread = new boost::thread(
			boost::bind(&PipelineStep::run_wrapper, this, threadno));
	}
}
...
FunPipelineStep::~FunPipelineStep() {}
