...
bool has_cycle_loop(state_id_t curr, Color* mark) const {
	...
	bool hasCycle = false;
	for_each_dest(curr, [=,&hasCycle](state_id_t, state_id_t t) {
...
//	printf("curr=%ld\n", (long)curr);
	...
	size_t num = 0;
	try {
		for_each_dest(curr, [&](state_id_t curr, state_id_t dest) {
...
size_t dag_total_path_len(size_t* num_words = NULL) const {
	assert(num_used_states() >= 1);
	valvec<state_id_t> rev_topo; rev_topo.reserve(total_states());
	...
	try {
		...
		for(size_t i = rev_topo.size(); i > 0; --i) {
			...
			size_t step_len = 1;
			if (is_pzip(parent)) {
				step_len += *get_zpath_data(parent);
			}
...
const {
	mark[curr] = Color::Grey;
	for_each_dest(curr, [&](state_id_t, state_id_t child) {
