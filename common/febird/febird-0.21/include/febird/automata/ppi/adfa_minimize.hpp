...
struct ADFA_Minimize_Context : boost::noncopyable {
	...
	typedef gold_hash_tab<state_id_t, state_id_t
		, HashEqByMap
		, febird_identity<state_id_t>
		, Layout
		> EquivalentRegister;
#ifdef FEBIRD_FSA_DEBUG
	...
	void collapse(const MyType* au, Au2& au2) {
		...
		for (size_t i = 0; i < min_size; ++i) {
			...
			au->for_each_move(curr,
				[&](state_id_t, state_id_t child1, auchar_t ch) {
					assert(ch < Au2::sigma);
...
struct ADFA_Minimize_ContextEx : super {
	const state_id_t* roots_beg;
	const state_id_t* roots_end;
	typename Au2::state_id_t* roots2_beg;

	ADFA_Minimize_ContextEx(const MyType* au
		, const valvec<state_id_t>& roots
		, valvec<typename Au2::state_id_t>* roots2
	) : super(au)
	{
...
struct ADFA_Minimize_Context2 : boost::noncopyable {
	...
	size_t  m_num_bucket;
	size_t  m_min_size;
	febird::AutoFree<state_id_t> m_base;
	...
	ADFA_Minimize_Context2(const MyType* au) {
		...
		m_base.p = (state_id_t*)malloc(sizeof(state_id_t) * n_all);
		if (NULL == m_base.p) {
			THROW_STD(runtime_error, "malloc");
		}
	...
	void collapse(const MyType* au, Au2& au2) {
		...
		minmap = (state_id_t*)realloc(minmap, fit_size);
		if (NULL == minmap) {
			THROW_STD(runtime_error, "realloc");
		}
		m_base.p = minmap;
		au2.resize_states(min_size);
		valvec<CharTarget<size_t> > children;
		...
		for(size_t parent1 = 0; parent1 < m_num_states; ++parent1) {
			...
			[&](size_t, size_t child1, auchar_t ch) {
				assert(ch < Au2::sigma);
...
void adfa_minimize_aux(Au2& au2) const {
	...
	au2.clear();
	MinContext ctx(this);
	topological_sort<MinContext&>(ctx);
	ctx.collapse(this, au2);
}

template<class MinContext, class Au2>
void
adfa_minimize_aux(const valvec<state_id_t>& roots, Au2& au2,
			  valvec<typename Au2::state_id_t>* roots2
) const {
#ifdef FEBIRD_FSA_DEBUG
	// must be sorted
...
#endif
	au2.clear();
	valvec<state_id_t> stack = roots;
	MinContext ctx(this, roots, roots2);
	topological_sort<MinContext&>(stack, ctx);
	ctx.collapse(this, au2);
...
template<class Au2>
void
adfa_minimize(const valvec<state_id_t>& roots, Au2& au2,
			  valvec<typename Au2::state_id_t>* roots2
) const {
...
template<class Au2>
void
adfa_minimize2(const valvec<state_id_t>& roots, Au2& au2,
			  valvec<typename Au2::state_id_t>* roots2
) const {
