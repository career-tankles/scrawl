#ifndef __febird_automata_linear_dfa3_hpp__
#define __febird_automata_linear_dfa3_hpp__

#include "dfa_algo.hpp"

template<int StateBytes>
class LinearDFA3 : public DFA_Interface {
...
public:
	...
	struct State {
		union u_ {
			struct state_ {
				link_uint_t  dest : StateBytes * 8 - 9;
				unsigned term_bit : 1;
				unsigned       ch : 8;
			} s;
			struct zpath_ {
	...
	static const state_id_t max_state = state_id_t((uint64_t(1) << (StateBytes*8 - 9)) - 1);
	...
	size_t mem_size() const override final { return sizeof(State) * states.size() + m_bits.bytes(); }
	...
	void build_from(const char* WalkerName, const Au& au, typename Au::state_id_t root = initial_state) {
		...
		throw std::invalid_argument("LinearDFA3::build_from: unknown WalkerName");
	}
	template<class OP>
...
#endif
	//	printf("LinearDFA3::get_zpath_data(%zd) = %02X: %.*s\n", s, *p, *p, p+1);
		return p;
	}
private:
	...
	void build_from_aux(const Au& au, typename Au::state_id_t root) {
		...
		if (idx >= max_state) {
			throw std::out_of_range("LinearDFA3::build_from_aux: idx exceeds max_state");
		}
		states.resize_no_init(idx);
		m_bits.resize_no_init(idx*2);
		walker.resize(au.total_states());
		walker.put_root(root);
		idx = 0;
		while (!walker.is_finished()) {
...
public:
	...
	template<class DataIO> void save_au(DataIO& dio) const {
		...
		dio.ensureWrite(m_bits.bldata(), m_bits.bytes());
	}
	template<class DataIO>
	friend void DataIO_loadObject(DataIO& dio, LinearDFA3& au) {
		typename DataIO::my_var_uint64_t version;
		dio >> version;
		if (version > SERIALIZATION_VERSION) {
			typedef typename DataIO::my_BadVersionException bad_ver;
			throw bad_ver(version.t, SERIALIZATION_VERSION, "LinearDFA3");
		}
		au.load_au(dio, version.t);
	}
	template<class DataIO>
	friend void DataIO_saveObject(DataIO& dio, const LinearDFA3& au) {
		dio << typename DataIO::my_var_uint64_t(SERIALIZATION_VERSION);
		au.save_au(dio);
	}

protected:
	void set_last_bit(size_t i, bool val) { m_bits.set(2*i+0, val); }
	void set_pzip_bit(size_t i, bool val) { m_bits.set(2*i+1, val); }
	void set_term_bit(size_t i, bool val) { states[i].u.s.term_bit = val; }
	bool get_last_bit(size_t i) const { return m_bits.is1(2*i+0); }
	bool get_pzip_bit(size_t i) const { return m_bits.is1(2*i+1); }
	bool get_term_bit(size_t i) const { return states[i].u.s.term_bit; }

	valvec<State> states;
	simplebitvec  m_bits;
	using DFA_Interface::zpath_states;
typedef LinearDFA3 MyType;
#include "ppi/dfa_match.hpp"
#include "ppi/dfa_const_virtuals.hpp"
//#include "ppi/post_order.hpp"
};

#endif // __febird_automata_linear_dfa3_hpp__
