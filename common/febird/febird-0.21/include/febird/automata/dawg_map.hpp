...
public:
	...
	explicit DAWG_Map(DFA_Interface* dfa) {
	...
	~DAWG_Map() {
		if (mmap_base) {
			...
			febird::mmap_close(mmap_fd, mmap_base);
		}
		delete dawg;
	}
	...
	void complete_dawg_map_mmap(fstring text_key_value_file
		...
		assert('\0' == *text_key_value_file.end());
		assert('\0' == *bin_dawg_file.end());
		DFA_Interface* dfa = DFA_Interface::load_mmap(bin_dawg_file.c_str());
		dawg = dynamic_cast<DAWG_Interface*>(dfa);
		if (NULL == dawg) {
			delete dfa;
...
//	void patch_values(FILE* text_fp, const boost::function<void(fstring line, fstring* pKey, Value* pVal)>& parse_line)
	...
	void load_mmap_values(fstring value_mmap_file) {
		assert(NULL != dawg);
		size_t len = 0;
	   	mmap_base = febird::mmap_load(value_mmap_file.c_str(), &mmap_fd, &len);
