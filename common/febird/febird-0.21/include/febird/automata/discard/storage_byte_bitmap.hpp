...
public:
    ...
    static const state_id_t nil_state = MyBytes == 6 ? UINT_MAX : state_id_t(uint64_t(1) << sbits) - 1;
    static const state_id_t  max_state = nil_state; // max_state <= null_state, and could be overridden
	...
	void setch(byte_t ch) {
	   	minch = ch;
		nbm = 0;
   	}
	void setMinch(byte_t ch) {
		assert(ch < minch);
		byte_t maxch = (0 == nbm) ? minch : getMaxch();
		nbm = (maxch - ch) / 32 + 1;
		minch = ch;
   	}
	void setMaxch(byte_t ch) {
		assert(ch > minch);
		assert(ch > getMaxch());
		nbm = (ch - minch) / 32 + 1;
   	}
	byte_t getMaxch() const {
	   	if (0 == nbm)
		   return minch;
	   	unsigned maxch = minch + (nbm*32 - 1);
		return std::min(255u, maxch);
   	}
	byte_t getMinch() const { return minch; }
	bool  is_single() const { return 0 == nbm; }
	bool  range_covers(byte_t ch) const { return ch == minch || (ch > minch && ch < minch + nbm*32); }
    ...
    void setpos(size_t tpos) {
        ...
        if (tpos >= size_t(nil_state) * MemPool::align_size) {
...
class State32 {
    ...
    byte_t   minch;  // min char
    byte_t   maxch;  // max char, inclusive
    unsigned term_bit : 1;
	unsigned pzip_bit : 1;
...
public:
	...
	void setch(byte_t ch) { minch = maxch = ch; }
	void setMinch(byte_t ch) { minch = ch; }
	void setMaxch(byte_t ch) { maxch = ch; }
	byte_t getMaxch() const { return maxch; }
	byte_t getMinch() const { return minch; }
	bool  is_single() const { return maxch == minch; }
	bool  range_covers(byte_t ch) const { return ch >= minch && ch <= maxch; }
...
public:
	...
	typedef valvec<std::map<byte_t, valvec<state_id_t> > > nfa_t;
...
//  static inline state_id_t nil_state() { return State::null_state; }
...
IF_DUP_PATH_POOL(x.set_pool_bit();) // is_free == (!is_pzip() && is_pool())
    ...
    Transition state_move(state_id_t curr, byte_t ch) const {
        assert(curr < (state_id_t)states.size());
		ASSERT_isNotFree(curr);
        return state_move(states[curr], ch);
	}
    Transition state_move(const State& s, byte_t ch) const {
    ...
    void add_move_checked(const state_id_t source, const state_id_t target, const byte_t ch) {
...
public:
	...
	nfa_t graph_reverse_nfa() const {
		...
		for (state_id_t s = 0; s < states.size(); ++s) {
			for_each_move(s, [&](state_id_t s, const Transition& t, byte_t c) {
	...
	CompactReverseNFA graph_reverse_nfa_opt() const {
...
#if 0 // for test
        valvec<std::map<byte_t, state_id_t> > cp(states.size());
        for (state_id_t s = 0; s < states.size(); ++s) {
        ...
        for (state_id_t s = 0; s < states.size(); ++s) {
            for (int ch = 0; ch < 256; ++ch) {
                ...
                state_id_t t2 = cp[s].count(ch) ? cp[s][ch] : nil_state;
                assert(t1 == t2);
            }
        }
#endif
	...
	valvec<CharTarget> trie_reverse_dfa() const {
		valvec<CharTarget> inv(states.size(), CharTarget(-1, nil_state));
		for (state_id_t s = 0; s < states.size(); ++s) {
			for_each_move(s, [&](state_id_t s, const Transition& t, byte_t c) {
			//	assert(isprint(c) || isspace(c));
				CharTarget& ct = inv[target_of(t)];
				assert(-1 == ct.c);
				assert(nil_state == ct.t);
				ct.c = c;
				ct.t = s;
			});
		}
    ...
    void for_each_move(state_id_t curr, OP op) const {
        ...
        if (is_32bitmap(bits)) {
            ...
            for (int pos = 0, i = 0; i < cBlock; ++i) {
                ...
                byte_t ch = s.getMinch() + i * hb.BlockBits;
                for (; bm; ++pos) {
    ...
    public:
        void operator()(state_id_t source, state_id_t target, byte_t ch) {
...
private:
	...
	template<class OnNthWord> struct ForEachWord_DFS {
        ...
        void operator()(state_id_t, const Transition& t, byte_t ch) {
			word.push_back(ch);
...
public:
    ...
    void graph_dfa_minimize(Automata<State2, Trans2>& minimized) const {
        assert(states.size() < (size_t)nil_state);
				  ...
				  Automata<State2, Trans2>& minimized) const
   	{
		...
		for (state_id_t s = 0; s < states.size(); ++s) {
			for_each_move(s, [&](state_id_t s, const Transition& t, byte_t c) {
				...
				assert(State2::nil_state == old || old == H.L[target_of(t)].blid);
			});
            if (states[s].is_term())
...
protected:
	...
	virtual void dot_write_one_transition(FILE* fp, long s, const Transition& t, byte_t ch) const {
...
public:
	...
	//	printf("%s:%d %s\n", __FILE__, __LINE__, BOOST_CURRENT_FUNCTION);
		...
		while (!dfs.is_finished()) {
			long i = dfs.next();
            for_each_move(i, [fp,this](state_id_t s, const Transition& t, byte_t c) {
   ...
   AddEos:
	...
	state_id_t make_path( state_id_t from, state_id_t to
        ...
        for (size_t j = 0; j < len-1; ++j) {
            if (nil_state == (p[j+1] = target_of(state_move(p[j], str[j])))) {
		...
		p[len] = (nil_state == old_to) ? to : old_to;
		return old_to;
	}
	...
	void remove_dead_states(CleanDFA& clean) {
		...
		if (numFreeStates) {
			...
			for (state_id_t f = firstFreeState; f != nil_state; ) {
				Free.set1(f);
		...
		valvec<state_id_t2> map(states.size(), (state_id_t2)clean.nil_state);
		for (size_t s = 0; s < states.size(); ++s) {
			if (mark.is1(s))
				map[s] = new_au_states++;
		}
		...
		for (size_t s = 0; s < states.size(); ++s) {
			if (map[s] == nil_state)
			   	continue;
			for_each_move(s, [&](state_id_t src, Transition dst, byte_t ch) {
				if (map[target_of(dst)] != clean.nil_state) {
	...
	public:
		...
		void getOnePath() {
            ...
            if (states[s].is_term() || !states[s].is_single())
				return;
			byte_t prev_ch = states[s].getMinch();
    ...
    void path_zip(ZippedAutomata& zipped, const char* BFS_or_DFS) const {
		...
		path_zip(is_confluence.bldata(), zipped, BFS_or_DFS);
	}
    template<class ZippedAutomata>
    ...
    void path_zip(const bit_access is_confluence, ZippedAutomata& zipped,
...
#else
	   	s2ds.resize(states.size(), ZippedAutomata::nil_state+0);
#endif
        ...
        while (!walker.is_finished()) {
			...
			this->for_each_move(walker.path.back(),
			  [&](state_id_t s1, transition_t t, byte_t c) {
    ...
    size_t add_zpath(state_id_t ss, const valvec<byte_t>& str) {
		return add_zpath(ss, &str[0], str.size());
	}
    size_t add_zpath(state_id_t ss, const byte_t* str, size_t len) {
        assert(len <= 255);
