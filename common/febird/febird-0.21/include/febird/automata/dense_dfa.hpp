#ifndef __febird_automata_dense_dfa_hpp__
#define __febird_automata_dense_dfa_hpp__
...
struct DenseState {
	typedef BitMap<Sigma, bm_uint_t> MyBitMap;
	enum { PoolAlign = sizeof(StateID) };
	static const StateID nil_state = StateID(-1);
	static const StateID max_state = StateID(-1);
...
template< class StateID, int Sigma = 256
	    , class State = DenseState<StateID, Sigma>
	   	>
class DenseDFA : public DFA_Interface, public DFA_MutationInterface
{
public:
	enum { PoolAlign = sizeof(StateID) };
	enum { char_bits = 8 };
	enum { sigma = Sigma };
    typedef MemPool<PoolAlign> MyMemPool;
...
public:
	...
	void del_move(size_t s, auchar_t ch) {
		assert(ch < Sigma);
        assert(s < (state_id_t)states.size());
		State& x = states[s];
		assert(!x.is_free);
...
public:
	...
	void for_each_dest_rev(state_id_t curr, OP op) const {
		...
		for (size_t i = n; i > 0; --i) op(curr, dests[i-1]);
	}
    template<class OP> // use ctz (count trailing zero)
	void for_each_dest(state_id_t curr, OP op) const {
		for_each_move(curr, [&](StateID s, StateID t, auchar_t){op(s,t);});
	}
    template<class OP> // use ctz (count trailing zero)
	...
	void compact() {
		...
		while (!walker.is_finished()) {
			...
			for_each_move(i, [&](StateID, StateID t, auchar_t c) {
					if (max_dest1 == t) {
						dense.set1(c);
					} else if (max_dest2 == t) {
						dense.set1(c);
	...
	const byte_t* get_zpath_data(size_t s) const {
		...
		return pool.data() + pos + sizeof(StateID) * n_sparse;
	}
...
public:
	...
	struct HashEq {
        ...
        HashEq(const DenseDFA* au) : pool(&au->pool) {}
...
size_t
DenseDFA<StateID, Sigma, State>::HashEq::hash(const State& x) const {
	assert(NULL != pool);
	assert(!x.is_pzip);
	size_t h = x.is_term;
	size_t n_sparse = 0;
	...
	h = x.dense_target1 * size_t(7) + h * 31;
	h = x.dense_target2 * size_t(7) + h * 31;
	if (n_sparse) {
		assert(nil_state != x.m_pos);
		size_t pos = x.getpos();
...
HashEq::equal(const State& x, const State& y) const {
	assert(NULL != pool);
...
const {
	...
	if (nil_state != x.dense_target1)
		h = Min[x.dense_target1] * size_t(7) + h * 31;
	if (nil_state != x.dense_target2)
		h = Min[x.dense_target2] * size_t(7) + h * 31;
	if (n_sparse) {
...
#endif // __febird_automata_dense_dfa_hpp__
