...
void DataIO_save_array_opt(DataIO& dio, const T* a, size_t n, Bswap bswap,
							 DataIO_is_realdump<DataIO, T, Size, Dumpable>)
{
...
template<class DataIO, class T, class Bswap>
void DataIO_save_elem_aux(DataIO& dio, const T& x, Bswap, boost::mpl::false_/*real_dumpable*/)
{
...
//--------------------------------------------------------
	Final_Output& operator<<(var_int30_t x)
	{
		this->getStream()->write_var_int30(x);
		return static_cast<Final_Output&>(*this);
	}

	Final_Output& operator<<(var_uint30_t x)
	{
...
#if !defined(BOOST_NO_INT64_T)
	Final_Output& operator<<(var_int61_t x)
	{
		this->getStream()->write_var_int61(x);
		return static_cast<Final_Output&>(*this);
	}
	Final_Output& operator<<(var_uint61_t x)
	{
...
//-----------------------------------------------------------------------------------
	Final_Output& operator<<(var_int30_t  x) { return this->operator<<(x.t); }
	Final_Output& operator<<(var_uint30_t x) { return this->operator<<(x.t); }

#if !defined(BOOST_NO_INT64_T)
	Final_Output& operator<<(var_int61_t  x) { return this->operator<<(x.t); }
	Final_Output& operator<<(var_uint61_t x) { return this->operator<<(x.t); }
#endif
};
...
public:
	...
	Final_Output& operator<<(const float x) {
		static_cast<Final_Output&>(*this) << *(const uint32_t*)(&x);
		return static_cast<Final_Output&>(*this);
	}
	Final_Output& operator<<(const double x) {
		static_cast<Final_Output&>(*this) << *(const uint64_t*)(&x);
		return static_cast<Final_Output&>(*this);
	}
	Final_Output& operator<<(const long double x) {
		// wire format of 'long double' is same as double
		static_cast<Final_Output&>(*this) << double(x);
		return static_cast<Final_Output&>(*this);
	}
	Final_Output& operator<<(const var_size_t x) {
		// default same with 'var_size_t_base'
		// can be overload and hidden by derived
		static_cast<Final_Output&>(*this) << static_cast<var_size_t_base>(x);
		return static_cast<Final_Output&>(*this);
	}
...
#define DATA_IO_GEN_DUMP_OUTPUT(Type)		\
	...
	Final_Output& operator<<(const std::vector<Type, Alloc>& x) {\
		static_cast<Final_Output&>(*this) << var_size_t(x.size());\
		if (febird_likely(!x.empty())) \
...
public:
	...
	template<class T>
	Final_Output& operator<<(const valvec<T>& x)
	{
		static_cast<Final_Output&>(*this) << var_size_t(x.size());
		DataIO_save_array(static_cast<Final_Output&>(*this), x.data(), x.size(), DATA_IO_BSWAP_FOR_BIG(T)());
		return static_cast<Final_Output&>(*this);
	}
...
public:
	...
	{
		static_cast<Final_Output&>(*this) << var_size_t(x.size());
		DataIO_save_array(static_cast<Final_Output&>(*this), x.data(), x.size(), DATA_IO_BSWAP_FOR_LITTLE(T)());
		return static_cast<Final_Output&>(*this);
	}
