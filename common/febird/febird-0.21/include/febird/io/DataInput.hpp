							 ...
							 DataIO_is_realdump<DataIO, T, Size, Dumpable> tag)
{
	...
	var_size_t size0; dio >> size0;
	x.resize(size0.t);
	if (febird_likely(0 != size0.t))
...
public:
	...
	Final_Input& operator>>(var_uint32_t& x)
   	{
	...
	Final_Input& operator>>(var_uint64_t& x)
   	{
	   	x.t = load_var_uint<uint64_t>();
	   	return static_cast<Final_Input&>(*this);
   	}
#endif
	Final_Input& operator>>(serialize_version_t& x)
   	{
	   	x.t = load_var_uint<uint32_t>();
	   	return static_cast<Final_Input&>(*this);
   	}
...
//-------------------------------------------------------------
	Final_Input& operator>>(var_int30_t& x)
	{
		x.t = this->getStream()->read_var_int30();
		return static_cast<Final_Input&>(*this);
	}
	Final_Input& operator>>(var_uint30_t& x)
	{
...
#if !defined(BOOST_NO_INT64_T)
	Final_Input& operator>>(var_int61_t& x)
	{
		x.t = this->getStream()->read_var_int61();
		return static_cast<Final_Input&>(*this);
	}
	Final_Input& operator>>(var_uint61_t& x)
	{
...
public:
	...
	Final_Input& operator>>(var_int30_t & x) { return *this >> x.t; }
	Final_Input& operator>>(var_uint30_t& x) { return *this >> x.t; }

#if !defined(BOOST_NO_INT64_T)
	Final_Input& operator>>(var_int61_t & x) { return *this >> x.t; }
	Final_Input& operator>>(var_uint61_t& x) { return *this >> x.t; }
#endif
};
...
public:
	...
	Final_Input& operator>>(std::string& x)
   	{
...
public:
	...
	Final_Input& operator>>(float& x) {
		static_cast<Final_Input&>(*this) >> *(uint32_t*)(&x);
		return static_cast<Final_Input&>(*this);
	}
	Final_Input& operator>>(double& x) {
		static_cast<Final_Input&>(*this) >> *(uint64_t*)(&x);
		return static_cast<Final_Input&>(*this);
	}
	Final_Input& operator>>(long double& x) {
		// wire format of 'long double' is same as double
		double y;
		static_cast<Final_Input&>(*this) >> y;
		x = y;
		return static_cast<Final_Input&>(*this);
	}
	Final_Input& operator>>(var_size_t& x) {
		// default same with 'var_size_t_base'
		// can be overload and hidden by derived
		static_cast<Final_Input&>(*this) >> static_cast<var_size_t_base&>(x);
		return static_cast<Final_Input&>(*this);
	}
...
public:
	...
	template<class T>
	Final_Input& operator>>(valvec<T>& x)
	{
		var_size_t size0; static_cast<Final_Input&>(*this) >> size0;
		x.resize(size0.t);
		DataIO_load_array(static_cast<Final_Input&>(*this), x.data(), size0.t, DATA_IO_BSWAP_FOR_BIG(T)());
		return static_cast<Final_Input&>(*this);
	}
...
public:
	...
	Final_Input& operator>>(valvec<T>& x)
	{
		var_size_t size0; static_cast<Final_Input&>(*this) >> size0;
		x.resize(size0.t);
		DataIO_load_array(static_cast<Final_Input&>(*this), x.data(), size0.t, DATA_IO_BSWAP_FOR_LITTLE(T)());
		return static_cast<Final_Input&>(*this);
	}
...
unsigned int DataIO_load_check_version(Input& in, unsigned int curr_version, const char* className)
{
	...
	in >> loaded_version;
	if (febird_unlikely(loaded_version.t > curr_version))
	{
// 		if (0 == className)
...
#define DATA_IO_OPTIMIZE_ELEMEN_LOAD(Self, Class, Members)\
	...
	{ febird::ByteSwapChain<Bswap>()Members; }		\
