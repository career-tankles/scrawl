...
//              _mm_popcnt_u32((unsigned int)(x >> 32));
    ...
    typedef uint64_t bm_uint_t;
#else
    typedef uint32_t bm_uint_t;
#endif
...
protected:
    BlockT  bm[BlockN];
public:
    BitMap(bool initial_value) {
        const BlockT e = initial_value ? ~BlockT(0) : 0;
...
public:
	...
	void  swap(simplebitvec& y) { bl.swap(y.bl); }
	      ...
	      bm_uint_t* risk_data()       { return &bl[0]; }
	const bm_uint_t* risk_data() const { return &bl[0]; }
};
