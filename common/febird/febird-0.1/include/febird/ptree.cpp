...
#endif
template<class Int>
size_t int_to_str(char* buf, Int x) {
...
#endif
size_t int_to_str(char* buf, bool x) {
	buf[0] = "01"[x?1:0];
	buf[1] = 0;
	return 1;
}
#else
  #define INT_TO_STR_IMPL(Format, Int) \
	size_t int_to_str(char* buf, Int x) { return sprintf(buf, #Format, x); }
INT_TO_STR_IMPL(%d  , int)
INT_TO_STR_IMPL(%ld , long)
INT_TO_STR_IMPL(%lld, long long)
INT_TO_STR_IMPL(%u  , unsigned int)
INT_TO_STR_IMPL(%lu , unsigned long)
INT_TO_STR_IMPL(%llu, unsigned long long)
#endif
