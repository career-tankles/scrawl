...
void len_distribute()
{
	...
	do {
		...
		if (febird_unlikely(nil == lenlink[k].head))
			lenlink[k].head = p;
		else
...
void distribute(ptrdiff_t first, ptrdiff_t kpos, ptrdiff_t cnt)
{
	...
	if (codefun) {
		do {
			...
			assert(k < radix);
			if (febird_unlikely(nil == keylink[k].head))
				keylink[k].head = p;
			else
...
void sort()
{
	if (febird_unlikely(0 == max_key_len))
		// not need sort
		return;
...
void rearrange_fast(Value* /*dummy*/)
{
	...
	for (i=0; i < N; ++i)
	{
		if (febird_unlikely(ov[i].addr != i))
		{
