...
int
febird_trb_compare_less_tag(const struct trb_vtab* vtab,
							const struct trb_tree* tree,
							const void* x,
							const void* y)
{
	fprintf(stderr, "fatal: febird_trb_compare_less_tag is being called\n");
	abort();
	return 0;
}
						...
						const void* key)
{
	...
	if (febird_likely(NULL != tree->trb_root))
	{
					  ...
					  struct trb_path_arg* arg)
{
	...
	if (febird_likely(NULL != tree->trb_root))
	{
		...
		if (febird_unlikely(k > TRB_MAX_HEIGHT)) {
					  ...
					  struct trb_path_arg* arg)
{
	...
	*/
	...
	if (febird_likely(tree->trb_root != NULL)) {
		TAG_set_child(p, dir);
...
trb_probe(const struct trb_vtab* vtab, struct trb_tree* tree, const void* data, int* existed)
{
	...
	if (febird_unlikely(NULL == n))
		return NULL;

	if (vtab->pf_data_copy)
	{
		if (febird_unlikely(!vtab->pf_data_copy(vtab, tree, vtab->data_offset + (char*)n, data)))
		{
...
int trb_erase(const struct trb_vtab* vtab, struct trb_tree* tree, const void* key)
{
	...
	if (febird_unlikely(NULL == tree->trb_root))
		return 0;
