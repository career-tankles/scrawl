...
struct DataIO_is_dump : public ::boost::mpl::false_
{
	...
	#define DataIO_IsDump_decltype(DataIO, T)
	...
	// support decltype
		#define DataIO_IsDump_decltype_decl(DataIO, T) \
			template<class dio>
			struct MyDIO_is_dump {
				typedef 
		#define DataIO_IsDump_decltype(DataIO, T) \
	#else
	#endif
...
#define DATA_IO_OPT_TRAITS_VERIFY(DataIO, T, Real_Dumpable) \
	BOOST_STATIC_ASSERT(Real_Dumpable::value || !(DataIO_is_dump<DataIO, T>::value))
