...
// ????????ģ??ֻ??Ϊ???? DATA_IO_LOAD_SAVE ??
// ???ɵ? Class::byte_swap_in ??ģ???ģ???û??ʹ?õ?ʱ?򣬲?ʵ????
...
public:
	...
	{
	//! ????????????????ʧ?ܣ??ܿ??????û??????????? DataIO_IsDump_TypeTrue1(T)
	//! ?? DataIO_is_realdump ?Ƶ??Ľ???ȴ?ǲ??? dump
	//  ?û?????Ϊ DATA_IO_DUMP_RAW_MEM ???????Ա????룬??ʱ DataIO_need_bswap::value == false
