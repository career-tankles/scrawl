...
public:
    ...
    static const state_id_t null_state = ~(state_id_t)0 >> 1;
    static const state_id_t  max_state = null_state;

    BS_State() {
        trans_000 = null_state;
		xpos = max_state;
		final_bit = 0;
		pzip_bit = 0;
    }
    ...
    void setpos(size_t tpos) {
        if (tpos >= size_t(null_state - Sigma) * MemPool::align_size) {
...
public:
	...
	state_id_t single_target(const State& s) const {
		...
		// default trans_000 is null_state, just the designated
