...
//	const static uint32_t null_state = 0x1FFFF; // 128K-1, Don't override null_state
	const static uint32_t max_output = 0x07FFF; //  32K-1
	typedef uint16_t word_id_t;
};
...
class Aho_Corasick : public BaseAutomata {
...
public:
    ...
    typedef typename state_t::word_id_t word_id_t; // smaller than state_id
    using super::states;
...
//	using super::TreeWalker;
	...
	word_id_t find(const fstring str, const Offset* offsets) const {
		...
		for (size_t i = 0; i < str.size(); ++i) {
			state_id_t next = super::state_move(curr, str.p[i]);
			if (null_state == next)
				return null_word;
			curr = next;
		}
	...
	size_t match_prefix(const fstring str 
		...
		//	assert(!s.is_pzip()); // DoubleArrayTrie.State has no is_pzip
			...
			if (str.size() == i)
				return i;
			state_id_t next = this->state_move(curr, tr(str.p[i]));
			if (null_state == next)
				return i;
	...
	template<class OnHitWord> //	__attribute__((flatten))
    void scan(const fstring str, OnHitWord on_hit_word) const {
		...
		scan_stream_imp<input_t, OnHitWord>(input, on_hit_word);
	}
...
//	__attribute__((flatten))
    void scan_stream(InputStream& input, OnHitWord on_hit_word) const {
		scan_stream_imp<InputStream&, OnHitWord>(input, on_hit_word);
	}

private:
	// pass input by value
	template<class InputStream, class OnHitWord>
    void scan_stream_imp(InputStream input, OnHitWord on_hit_word) const {
		assert(!output.empty());
        state_id_t curr = initial_state;
		for (size_t pos = 0; !input.empty(); ++pos) {
            char_t c = *input; ++input;
            state_id_t next;
            while ((next = this->state_move(curr, c)) == null_state) {
...
public:
	...
	void operator()(state_id_t parent, state_id_t curr, char_t c) const {
		state_id_t back = ac->ffail(parent);
		state_id_t next;
		while ((next = ac->state_move(back, c)) == null_state) {
...
public:
	...
	word_id_t& add_word(const fstring key) {
		...
		size_t j = 0;
		while (j < key.size()) {
			state_id_t next = ac->state_move(curr, key.p[j]);
			if (null_state == next)
				goto AddNewStates;
	...
	AddNewStates:
		do {
			...
			ac->add_move_checked(curr, next, key.p[j]);
			curr = next;
			++j;
		} while (j < key.size());
    ...
    //            (long)tmpOut.size(), (long)ac->states.size());
        ...
        dfa_algo(ac).bfs_tree(initial_state, BFS_AC_Builder(ac, &tmpOut));
	...
	void set_word_id_as_lex_ordinal(valvec<Offset>* offsets, valvec<char>* strpool) {
		if (strpool)
			strpool->resize(0);
		offsets->resize(0);
		offsets->reserve(num_words+1);
		if (strpool)
			strpool->reserve(all_words_size);
		Offset curr_offset = 0;
		dfa_algo(ac).for_each_word(
		...
		offsets->unchecked_push_back(curr_offset);
		assert(offsets->size() == num_words+1);
	...
	void sort_by_word_len(const WordAttributs* beg, const WordAttributs* end, GetOffset get_offset) {
		assert(num_words + 1  == size_t(end-beg));
