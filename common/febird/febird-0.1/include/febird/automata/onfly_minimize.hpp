...
public:
    ...
    struct Ordered_Real_Map_Register {
		...
		std::pair<size_t, bool> insert_i(size_t state_id) {
	...
	struct Parallel_Register {
		...
		std::pair<size_t, bool> insert_i(state_id_t state_id) {
		//	printf("insert_i(state_id=%u)\n", state_id);
		...
		void resize(size_t new_size) { reg.get_keyof().resize(new_size); }
	...
	struct UnOrdered_Parallel_Register : Parallel_Register<unordered_entry> {
		...
		void compute_indegree(const Au* au, state_id_t RootState) {
			this->reg.get_keyof().resize(au->total_states());
		...
		void resize_indegree(size_t new_size) { this->resize(new_size); }
	};
...
class DFA_OnflyMinimize<Au>::UnOrdered_Builder {
	void clone_path(ptrdiff_t ConfluenceIndex, ptrdiff_t nPrefix, const char* str) {
		...
		for (ptrdiff_t j = ConfluenceIndex; j <= nPrefix; ++j) {
			...
			assert(null_state != old); (void)old;
		}
...
public:
    ...
    bool add_word(const fstring str) {
		...
		path.fill((state_id_t)null_state);
		path[0] = RootState;
...
L1:
		...
		for (ptrdiff_t j = len; j > 0; --j) {
			...
			if (au->get_single_child(curr) == null_state &&
			   !au->is_term(curr))
		   	{
				assert(1 == R.indegree(curr));
				assert(au->has_single_child(curr));
				...
				std::pair<size_t, bool> ib = R.insert_i(curr);
...
private:
	void clone_path2(size_t ConfluenceIndex, const fstring str) {
		if (ConfluenceIndex > 1)
			R.erase(path[ConfluenceIndex-1]); // erase for future update
		for (size_t j = ConfluenceIndex; j < path.size(); ++j) {
			...
			if (j <= str.size()) {
				...
				assert(null_state != au->get_single_child(path[j-1]));
	...
	size_t del_by_prefix_start(const fstring prefix) {
	...
	size_t del_by_prefix_finish(const fstring prefix) {
...
L1:
			   ...
			   !au->is_term(curr))
		   	{
				...
				if (!ib.second) { // existed an equivalent state
					...
					if (j <= prefix.n)
						...
						assert(au->has_single_child(path[j-1]));
	...
	valvec<char> m_suffix;
public:
	size_t del_by_prefix(const fstring prefix) {
		auto sure = [](const fstring) { return true; };
	...
	size_t del_by_prefix(const fstring prefix, Pred pred) {
		...
		if (1 != ret)
			return ret;
		bool sure = pred(fstring(m_suffix.data(), m_suffix.size()-1));
		if (!sure)
...
public:
	///@note
	///  delim must not be in any call of [str, len)
	void add_suffix_adfa(const fstring str, char_t delim, state_id_t root2) {
		ptrdiff_t len = str.n;
		assert((NULL != str.p && len > 0) || 0 == len);
		path.resize_no_init(len + 1);
		path.fill((state_id_t)null_state);
...
private:
	void minimize_update_path(const fstring str) {
		for (ptrdiff_t j = str.n; j >= 0; --j) {
			std::pair<size_t, bool> ib = R.insert_i(path[j]);
			if (!ib.second) { // existed an equivalent state
				state_id_t EqState = ib.first;
				if (EqState == path[j])
					break;
	...
	// now indegree serve as 'color' flag for DFS
		...
		inv_nfa.index.unchecked_push_back({null_state+0, 0});
		...
		for (size_t j = len+1, n = path.size(); j < n; ++j) {
			...
			std::pair<size_t, bool> ib = R.insert_i(child);
			if (!ib.second) { // existed an equivalent state
				state_id_t EqState = ib.first;
				if (EqState == child)
					break;
...
public:
	...
	void reset_root_state(state_id_t RootState) {
		...
		this->RootState = RootState;
		assert(RootState < au->total_states());
		assert(au->has_single_child(RootState));
		assert(au->get_single_child(RootState) == null_state);
		R.resize(au->total_states());
	//	R_init(R, au, RootState);
	...
	//	assert((NULL != str.p && len > 0) || 0 == len);
        ...
        ptrdiff_t nPrefix; // length of prefix
		const char* str0 = last_word.data();
		path.resize_no_init(max_len + 1);
		for (nPrefix = 0; nPrefix < min_len; ++nPrefix) {
			if (str[nPrefix] != str0[nPrefix]) {
				if ((char_t)str[nPrefix] < (char_t)str0[nPrefix]) {
