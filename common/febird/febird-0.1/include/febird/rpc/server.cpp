...
{
	m_thread.reset(new boost::thread(boost::bind(&session_base::run, this)));
}
...
rpc_server_base::~rpc_server_base()
{
	for (int nRetry = 0; nRetry < 100; ++nRetry) {
		long remainSessions;
		{
			boost::mutex::scoped_lock lock(m_mutex_sessionList);
...
{
	boost::mutex::scoped_lock lock(m_mutex_globaleScopeObjects);
	assert(servant);
