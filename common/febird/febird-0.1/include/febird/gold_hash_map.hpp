...
public:
	...
	typedef typename NodeLayout::const_iterator const_iterator;
...
//	void resize(size_t n) { rehash(n); }
	...
	size_t next_i(size_t idx) const { assert(idx < nElem); return idx+1;}
	size_t capacity() const { return maxElem; }
	size_t delcnt() const { return freelist_size; }

	      iterator  begin()       { return m_nl.begin(); }
	const_iterator  begin() const { return m_nl.begin(); }
	const_iterator cbegin() const { return m_nl.begin(); }

	      iterator  end()       { return m_nl.begin() + nElem; }
	const_iterator  end() const { return m_nl.begin() + nElem; }
	const_iterator cend() const { return m_nl.begin() + nElem; }
	...
	std::pair<iterator, bool> insert(const CompatibleObject& obj) {
		...
		return std::pair<iterator, bool>(m_nl.begin() + ib.first, ib.second);
	}
	      iterator find(const Key& key)       { return m_nl.begin() + find_i(key); }
	const_iterator find(const Key& key) const { return m_nl.begin() + find_i(key); }
...
public:
	...
	template<class Compare>	void sort(Compare comp) {
		if (freelist_size)
			revoke_deleted_no_relink();
		febird_parallel_sort(begin(), end(), Compare(comp));
		relink_fill();
		is_sorted = true;
	}
