...
class PipelineStep::queue_t : public base_queue
{
public:
...
PipelineStep::PipelineStep(int thread_count)
: m_owner(0)
{
	if (0 == thread_count)
	{
	...
	m_threadsData.resize(thread_count);
}
...
int PipelineStep::getInQueueSize() const {
	assert(m_prev->m_out_queue);
	return m_prev->m_out_queue->size();
}

int PipelineStep::getOutQueueSize() const {
	assert(this->m_out_queue);
	return this->m_out_queue->size();
}

void PipelineStep::setOutQueueSize(int size) {
	if (size > 0) {
...
{
	return m_threadsData[threadno].m_err_text;
}
...
inline bool PipelineStep::isPrevRunning()
{
...
bool PipelineStep::isRunning()
{
	...
	{
		if (m_threadsData[t].m_run)
			return true;
	}
	return false;
}

void PipelineStep::wait()
{
...
void PipelineStep::start(int queue_size)
{
	...
	for (size_t threadno = 0; threadno != m_threads.size(); ++threadno)
	{
		// 在 PipelineThread 保存 auto_ptr<boost::thread> 指针
		// 如果直接内嵌一个 boost::thread 实例，那么在 new PipelineThread 时，该线程就开始运行
		// 有可能在 PipelineThread 运行结束时，m_threads[threadno] 还未被赋值，导致程序崩溃
		// 所以分两步，先构造对象，并且对 m_threads[threadno] 赋值，然后再运行线程
		//
		//
		// hold auto_ptr<boost::thread> in PipelineThread
		//
		// if direct embed a boost::thread in PipelineThread, when new PipelineThread,
		...
		//
		m_threads[threadno] = new boost::thread(boost::bind(&PipelineStep::run_wrapper, this, threadno));
	}
}

void PipelineStep::onException(int threadno, const std::exception& exp)
{
	static_cast<string_appender<>&>(m_threadsData[threadno].m_err_text = "")
...
// 			  << ", threadno[" << threadno
// 			  << "], caught: " << m_threadsData[threadno].m_err_text
// 			  << std::endl;
}
...
void PipelineStep::run_wrapper(int threadno)
{
	m_threadsData[threadno].m_run = true;
	...
	m_owner->stop();
	m_threadsData[threadno].m_run = false;
}

void PipelineStep::run(int threadno)
{
	...
	if (this == m_owner->m_head->m_next
		...
		case ple_keep:
			...
			if (this == m_owner->m_head->m_prev)
				run_serial_step_fast(threadno, &PipelineStep::serial_step_do_last);
			else
				run_serial_step_fast(threadno, &PipelineStep::serial_step_do_mid);
			break;
		}
	}
}

void PipelineStep::run_step_first(int threadno)
{
...
void PipelineStep::run_step_last(int threadno)
{
...
void PipelineStep::run_step_mid(int threadno)
{
...
void PipelineStep::run_serial_step_slow(int threadno,
...
void PipelineStep::run_serial_step_fast(int threadno,
								   void (PipelineStep::*fdo)(PipelineQueueItem&)
								   )
{
	assert(ple_keep == m_pl_enum);
	...
	while (isPrevRunning()) {
		...
		while (cache[head].plserial == m_plserial) {
			...
			++m_plserial;
			head = (head + 1) % nlen;
			if (febird_unlikely(!overflow.empty() && overflow[0].plserial == m_plserial)) {
				// very rare case
...
void FunPipelineStep::process(int threadno, PipelineQueueItem* task)
{
...
{
public:
	Null_PipelineStep() : PipelineStep(0) {}
protected:
...
void PipelineProcessor::start()
{
	...
	for (PipelineStep* s = m_head->m_next; s != m_head; s = s->m_next)
	{
		if (PipelineStep::ple_generate == s->m_pl_enum)
		{
			assert(-1 == plgen);  // only one generator
			assert(-1 == plkeep); // keep must after generator
			plgen = nth;
		}
		else if (PipelineStep::ple_keep == s->m_pl_enum)
		{
...
{
	m_head->m_out_queue = new PipelineStep::queue_t(input_feed_queue_size);
	start();
}
